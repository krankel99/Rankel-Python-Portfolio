...
# Function to rebuild the EntityRuler pipeline with current patterns
...
# Initial build of the pipeline with existing patterns
rebuild_pipeline()
...
st.header('Define Custom Entity Patterns') # moved this section up for better UX
...
if st.button('Add Pattern'):
    ...
    if custom_label.strip() == "" or custom_pattern.strip() == "":
        ...
        rebuild_pipeline()  # Update spaCy pipeline with new patterns
...
if st.button('Clear All Patterns'):
    ...
    rebuild_pipeline()  # Reset spaCy pipeline

    st.success('All custom patterns have been cleared!')
    st.markdown('If users press the clear all patterns once, it will be removed from the patterns used below, but still displayed in the custom patterns section above. To get rid of this, please press the button twice.')


# Process text with current pipeline
st.header('Labeled Entities')
doc = nlp(user_text)
